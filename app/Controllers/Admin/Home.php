...
/**
 * Administration Home Controller Class
 * 관리자 홈 컨트롤러 클래스
 *
...
class Home extends BaseController
{
    ...
    public function initController(RequestInterface $request, ResponseInterface $response, LoggerInterface $logger)
    {
        ...
        $this->data['jquery_js'] = adminThemeUrl('assets/jquery.min.js');
        $this->data['zxcvbn_js'] = adminThemeUrl('assets/zxcvbn.js');
    }
	...
	{
        return $this->theme->renderAdminPage('index');
	}
    ...
    public function finish()
    {
        ...
        $settings->set( Settings::users_default_ur_id, $userroles->getIdFromField($userroles->_ur_name, 'general') );

        // Add Super Administrator in users table
        $admin_data = [
            UsersModel::u_login => $data['adminid'],
            UsersModel::u_pass => $data['adminpass'],
            UsersModel::u_ur_id => $userroles->getIdFromField($userroles->_ur_name, 'superadmin'),
            UsersModel::u_nickname => $data['adminname'],
            UsersModel::u_email => $data['adminemail'],
        ];
        $result = $users->addUser($admin_data);
        if (!$result) {
            $errors = $users->errors();
            $error_data = null;
            if ( is_array($errors) ) {
